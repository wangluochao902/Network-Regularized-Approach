...
def get_args():
    parser = argparse.ArgumentParser(description="Run the network regularized approach.")
    parser.add_argument('--num_workers', default=20, type=int, help='number of processes to train with')
    ...
    parser.add_argument('--sleep_mean', default=1, type=float, help='learning rate')
    parser.add_argument('--a', default=10, type=float, help='attraction')
    parser.add_argument('--graph_seed', default=1, type=int, help='random seed to generate the graph')
    parser.add_argument('--k', default=8, type=int, help='each worker is connected with k nearest '
                                                         'neighbors in watts_strogatz_graph')
    parser.add_argument('--print_steps', default=100, type=int, help='print out result to screen every print_steps')
    parser.add_argument('--momentum', default=0.9, type=float)
    parser.add_argument('--steps', default=30000, type=int, help='number of steps')
    ...
    parser.add_argument('--store_step', default=10, type=int, help='calculate cen_to_opt, apend it to list every store_step')
    parser.add_argument('--save_step', default=10000, type=int, help='save cen_to_opts to file every save_step')
...
def get_accu_and_loss(ps, args):
    ...
    while True:
        ...
        all_weights = np.array([ray.get(all_weights_ids[i]) for i in range(args.num_workers)])
        ...
        if the_time > 5:
            ...
            np.save(args.save_dir+'flocking_num_worker%d, k_%d, round_%d' % (args.num_workers, args.k, args.round), np.array(value))
...
def worker_task(ps, current_worker_index, args):
    ...
    while step < args.steps and time.time() - start_time < args.stop_time:
        ...
        time.sleep(np.random.exponential(args.sleep_mean))
        ...
        if step % 100 == 0 and current_worker_index == 0:
            print('step', step, 'elapsed_time', time.time() - start_time, 'training loss is', loss_value)


if __name__ == "__main__":
    args = get_args()
    ray.init(num_cpus=args.num_workers+1)
    args.save_dir = './flocking_log_%.1f/' % args.sleep_mean
    ...
    graph = construct_graph_watts(args.num_workers, args.k, seed=args.graph_seed)
    print(graph)
    ps = ParameterServer.remote(num_workers=args.num_workers, weights_ids=weights_ids, graph=graph)
    worker_tasks = [worker_task.remote(ps, i, args) for i in range(args.num_workers)]
