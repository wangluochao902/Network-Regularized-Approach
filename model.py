...
class SimpleCNN(object):
        ...
        with tf.Graph().as_default():
            ...
            with tf.name_scope("adam_optimizer"):
                if args.momentum is not None:
                    self.optimizer = tf.train.MomentumOptimizer(args.lrn, args.momentum)
                else:
                    self.optimizer = tf.train.GradientDescentOptimizer(args.lrn)
            ...
            self.sess = tf.Session(
                config=tf.ConfigProto(
                    intra_op_parallelism_threads=args.threads_per_worker,
    ...
    def minimize(self, x, y):
        loss_value, _ = self.sess.run([self.cross_entropy, self.train_step],
                                      feed_dict={self.x: x,
                                                 self.y_: y,
                                                 self.keep_prob: 0.5})
        new_weights = self.get_flat()
        return loss_value, new_weights
